...
#SDK_BASE	?= c:/Espressif/ESP8266_NONOS_SDK-3.0
...
MODULES		= driver fat mqtt user
EXTRA_INCDIR	= include $(EXTRA_BASE)/include libesphttpd/include fat/include mqtt/include
...
CFLAGS		= -Os -g -O2 -std=gnu90 -Wpointer-arith -Wundef -Werror -Wl,-EL -fno-inline-functions \
				-nostdlib -mlongcalls -mtext-section-literals -mno-serialize-volatile \
...
CFLAGS += -DOTA_FLASH_MAP=$(SPI_SIZE_MAP)
endif
...
ESPTOOL_SIZE=$(call maplookup,$(SPI_SIZE_MAP),0:4m 1:2m 2:8m 3:16m 4:32m 5:16m-c1 6:32m-c1)
...
.PHONY: all checkdirs clean libesphttpd default-tgt flash rebuild 
...
clean:
	...
	$(Q) $(MAKE) -C libesphttpd/mkupgimg/ clean
	$(Q) rm -f $(APP_AR)
	$(Q) rm -f $(TARGET_OUT)
	$(Q) rm -rf $(BUILD_BASE)
	$(Q) rm -rf $(FW_BASE)
