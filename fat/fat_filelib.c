...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR FL_FILE* _allocate_file(void)
{
	//os_printf("in allocate file\n");
	int fileIndex = 0;
	while (_files[fileIndex++].fileInUse != fileNotBeingUsed);
	if (fileIndex < FATFS_MAX_OPEN_FILES)
	{
		_files[--fileIndex].fileInUse = fileBeingUsed;
		//os_printf("in allocate_file struct address returned = %x fileIndex =%d\n", &_files[fileIndex],fileIndex);
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR int _check_file_open(FL_FILE* file)
{
	return 0;
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR void _free_file(FL_FILE* file)
{
    // Remove from open list
	os_printf("in Free File address freed=%x\n", file);
	file->fileInUse = fileNotBeingUsed;
}
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR int _open_directory(char *path, uint32 *pathCluster)
{
...
static ICACHE_FLASH_ATTR int _create_directory(char *path)
{
    ...
    file = _allocate_file();
	os_printf("in create after allocate  file file name =%s\n", file->filename);
    if (!file)
        return 0;
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR FL_FILE* _open_file(const char *path)
{
...
//-----------------------------------------------------------------------------
#if FATFS_INC_WRITE_SUPPORT
static ICACHE_FLASH_ATTR FL_FILE* _create_file(const char *filename)
{
	//os_printf("in create 0 filename = %s\n", filename);
    FL_FILE* file;
    struct fat_dir_entry sfEntry;
    char shortFilename[FAT_SFN_SIZE_FULL];
    int tailNum = 0;

	//os_printf("in create_file 1\n\n");
	...
	//os_printf("in create file  allocated file address  = %x\n", &file);
	//os_printf("in create file  allocated file address  = %x\n", file);
    if (!file)
        return NULL;

	//os_printf("in create_file 2\n\n");
    ...
    if (fatfs_split_path((char*)filename, file->path, sizeof(file->path), file->filename, sizeof(file->filename)) == -1)
    {
		...
		//os_printf("in create_file 2a\n\n");

        _free_file(file);
        return NULL;
    }

	//os_printf("in create_file 3 filename = \n\n",file->filename);

    // Check if file already open
    if (_check_file_open(file))
    {

		//os_printf("in create_file 4\n\n");

        _free_file(file);
        return NULL;
    }

	//os_printf("in create_file 5\n\n");
    ...
    fatfs_fat_purge(&_fs);
	//os_printf("in create file file name = %s\n", file->filename);
    return file;
}
#endif
//-----------------------------------------------------------------------------
...
static uint32 ICACHE_FLASH_ATTR _read_sectors(FL_FILE* file, uint32 offset, uint8 *buffer, uint32 count)
{
    ...
    uint32 i;
    uint32 lba;
	//os_printf("in _read_sectors offset ==%x\n", offset);
    ...
    else
    {
        ...
        {
			//os_printf("in _read_sectors--2\n\r");
            // Set start of cluster chain to initial value
            i = 0;
            Cluster = file->startcluster;
        }

		//os_printf("in _read_sectors cluster idx = %x\n\r", ClusterIdx);

        // Follow chain to find cluster to read
        for ( ;i<ClusterIdx; i++)
        {
			//os_printf("in _read_sectors--3\n\r");
            uint32 nextCluster;
...
void ICACHE_FLASH_ATTR fl_init(void)
{
	...
	{
		_files[i].fileInUse = fileNotBeingUsed;
		os_printf("&_files[%d] = %x\n",i, &_files[i]);
	}
...
// fl_attach_media:
//-----------------------------------------------------------------------------
int ICACHE_FLASH_ATTR fl_attach_media( void)
{
...
void* ICACHE_FLASH_ATTR fl_fopen(const char *path, const char *mode)
{
    ...
    if (!(flags & FILE_READ))
        if ((flags & FILE_CREATE) && !file)
			...
			{
				file = _open_file(path);
				os_printf("Write existing file create hit\n");
			}
	if (file)
	{
		file->flags = flags;
...
//-----------------------------------------------------------------------------
#if FATFS_INC_WRITE_SUPPORT
ICACHE_FLASH_ATTR static uint32 _write_sectors(FL_FILE* file, uint32 offset, uint8 *buf, uint32 count)
{
	...
	//os_printf("in _write_sectors count ==%x\n\r", count);
	...
	//os_printf("in _write_sectors filename =%s\n",file->filename);
	//os_printf("in _write_sectors ClusterIdx==%x    file->last_fat_lookup.ClusterIdx  == %x\n\r", ClusterIdx, file->last_fat_lookup.ClusterIdx);
    ...
    else
    {
        ...
        if (ClusterIdx && ClusterIdx == file->last_fat_lookup.ClusterIdx + 1)
        {
            i = file->last_fat_lookup.ClusterIdx;
            Cluster = file->last_fat_lookup.CurrentCluster;
			//os_printf("in _write_sectors cluster1 = %x\n\r", Cluster);
        }
        // Start searching from the beginning..
        else
        {
            // Set start of cluster chain to initial value
			//os_printf("in _write_sectors starting from beginning loop  ??\n\r", Cluster);
            i = 0;
            Cluster = file->startcluster;
        }
        ...
        for ( ;i<ClusterIdx; i++)
        {
			...
			//os_printf("in _write_sectors in follow the chain ClusterIdx  ==%x\n\r", ClusterIdx);
            uint32 nextCluster;
            ...
            if (!fatfs_cache_get_next_cluster(&_fs, file, i, &nextCluster))
            {
				...
				//os_printf("in write sector  cache loop  cluster ==%x\n", Cluster);
                // Scan file linked list to find next entry
                nextCluster = fatfs_find_next_cluster(&_fs, Cluster);
				//os_printf("in write sector  cache loop  nextCluster ==%x\n", nextCluster);
                // Push entry into cache
                fatfs_cache_set_next_cluster(&_fs, file, i, nextCluster);
            }
...
void ICACHE_FLASH_ATTR fl_fclose(void *f)
{
    ...
    if (file)
    {
        ...
        file->file_data_dirty = 0;
        file->filelength_changed = 0;
	//	file->last_fat_lookup.ClusterIdx = 0xFFFFFFFF;    //robert
	//	file->last_fat_lookup.CurrentCluster = 0xFFFFFFFF;  //robert

	//	fatfs_cache_init(&_fs, file);  //robert

	//	fatfs_fat_purge(&_fs);  //robert
...
int ICACHE_FLASH_ATTR fl_fwrite(const void * data, int size, int count, void *f)
{
    ...
    while (bytesWritten < length)
    {
        ...
        if ((offset == 0) && ((length - bytesWritten) >= FAT_SECTOR_SIZE))
        {
			...
			//os_printf("in fl_fwrite  length == %x    byteswritten =  %x\n",length,bytesWritten);


			//os_printf("in fl write1 sector= %x\n", sector);
