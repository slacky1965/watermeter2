...
int ICACHE_FLASH_ATTR fatfs_add_free_space(struct fatfs *fs, uint32 *startCluster, uint32 clusters)
{
	...
	//os_printf("in fatfs_add_free_space start cluster == %x\n\n", *startCluster);
    // Set the next free cluster hint to unknown

	//os_printf("in fatfs_add_free_space start fs->next_free_cluster == %x\n\n", fs->next_free_cluster);
 ...
 //  if (fs->next_free_cluster != FAT32_LAST_CLUSTER)
  //      fatfs_set_fs_info_next_free_cluster(fs, FAT32_LAST_CLUSTER);
	...
	//os_printf("in fatfs_add_free_space clusters == %x\n\n", clusters);

    for (i=0;i<clusters;i++)
    {
        // Start looking for free clusters from the beginning
      //  if (fatfs_find_blank_cluster(fs, fs->rootdir_first_cluster, &nextcluster))    BOB

		//os_printf("in fatfs_add_free_space start2 ===%x\n", start);
		if (fatfs_find_blank_cluster(fs, start, &nextcluster))   //bob  start
        {
            // Point last to this
			...
			fatfs_set_fs_info_next_free_cluster(fs, nextcluster);  // at this point this is the best starting point to find a spare cluster
        }															//fatfs_find_blank_cluster will use this to find future spare
        else
            return 0;
    }
...
int ICACHE_FLASH_ATTR fatfs_allocate_free_space(struct fatfs *fs, int newFile, uint32 *startCluster, uint32 size)
{
    ...
    uint32 clusterCount;
    uint32 nextcluster;
	//os_printf("in fatfs_allocate_free_space size = %x\n", size);
    if (size==0)
        return 0;
 ...
 // if (fs->next_free_cluster != FAT32_LAST_CLUSTER)
  //  fatfs_set_fs_info_next_free_cluster(fs, FAT32_LAST_CLUSTER);
    ...
    if (newFile)
    {
        ...
        if (clusterCount==1)
        {
            ...
            *startCluster = nextcluster;
			...
			//fatfs_set_fs_info_next_free_cluster(fs, nextcluster);
			return 1;
        }
    }
    ...
    if (!fatfs_add_free_space(fs, &nextcluster, clusterCount))
            return 0;
//	os_printf("in fatfs_allocate_free_space clusterCount = %x\n", clusterCount);
    return 1;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR int fatfs_find_free_dir_offset(struct fatfs *fs, uint32 dirCluster, int entryCount, uint32 *pSector, uint8 *pOffset)
{
...
int ICACHE_FLASH_ATTR fatfs_add_file_entry(struct fatfs *fs, uint32 dirCluster, char *filename, char *shortfilename, uint32 startCluster, uint32 size, int dir)
{
    ...
    while (1)
    {
        ...
        if (fatfs_sector_reader(fs, dirCluster, x++, 0))
        {
            ...
            if (dirtySector)
            {
                if (!media_write(fs->currentsector.address, fs->currentsector.sector, 1))
                    return 0;
