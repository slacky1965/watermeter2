...
int ICACHE_FLASH_ATTR fatfs_init(struct fatfs *fs)
{
	...
	//os_printf("in fatfs init\n");
	if (!media_read(0, fs->currentsector.sector, 1))
	{
	...
	//fatfs_print_sector(0, fs->currentsector.sector);


	//print_sector(0, fs->currentsector.sector);
    ...
    {
		//os_printf("switch =%x\n ", fs->currentsector.sector[PARTITION1_TYPECODE_LOCATION]);
    ...
    if (!media_read(fs->lba_begin, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;


	
	//print_sector(fs->lba_begin, fs->currentsector.sector);
	...
	//os_printf("sectors per cluster == %x\n", fs->sectors_per_cluster);


    reserved_sectors = GET_16BIT_WORD(fs->currentsector.sector, BPB_RSVDSECCNT);
	
	//os_printf("reserved_sectors == %x\n", reserved_sectors);


    num_of_fats = fs->currentsector.sector[BPB_NUMFATS];

	//os_printf("num_of_fats == %x\n", num_of_fats);



    fs->root_entry_count = GET_16BIT_WORD(fs->currentsector.sector, BPB_ROOTENTCNT);

	//os_printf("root_entry_count == %x\n", fs->root_entry_count);
	...
	//os_printf("fs->rootdir_first_cluster == %x\n", fs->rootdir_first_cluster);


    fs->fs_info_sector = GET_16BIT_WORD(fs->currentsector.sector, BPB_FAT32_FSINFO);

	//os_printf("fs->fs_info_sector = = %x\n", fs->fs_info_sector);
	...
	//os_printf("fs->rootdir_first_sector = = %x\n", fs->rootdir_first_sector);

    fs->rootdir_sectors = ((fs->root_entry_count * 32) + (FAT_SECTOR_SIZE - 1)) / FAT_SECTOR_SIZE;

	//os_printf(" fs->rootdir_sectors = = %x\n", fs->rootdir_sectors);

    // First FAT LBA address
    fs->fat_begin_lba = fs->lba_begin + reserved_sectors;

	//os_printf(" fs->fat_begin_lba == %x\n", fs->fat_begin_lba);


    // The address of the first data cluster on this volume
    fs->cluster_begin_lba = fs->fat_begin_lba + (num_of_fats * fs->fat_sectors);

	//os_printf(" fs->cluster_begin_lba= %x\n", fs->cluster_begin_lba);
...
{
	//os_printf("in fatfs_sector_read\n\r");
    return media_read(lba, target, count);
}
//-----------------------------------------------------------------------------
// fatfs_sector_write:
//-----------------------------------------------------------------------------
int ICACHE_FLASH_ATTR fatfs_sector_write(struct fatfs *fs, uint32 lba, uint8 *target, uint32 count)
{
	...
	//os_printf("in fatfs_sector_write  count ==%x\n\r",count);
    return media_write(lba, target, count);
}
//-----------------------------------------------------------------------------
...
int ICACHE_FLASH_ATTR fatfs_sector_reader(struct fatfs *fs, uint32 start_cluster, uint32 offset, uint8 *target)
{
	...
	//os_printf("in fatfs_sector_reader\n");
    // User provided target array
    if (target)
		return media_read(lba, target, 1);
    // Else read sector if not already loaded
    else if (lba != fs->currentsector.address)
    {
        fs->currentsector.address = lba;
		return media_read(fs->currentsector.address, fs->currentsector.sector, 1);
    }
    else
        return 1;
}
...
{
	//os_printf("in fatfs_read_sector \n");
    // FAT16 Root directory
    if (fs->fat_type == FAT_TYPE_16 && cluster == 0)
    {
		...
		{
			//os_printf("in fatfs_read_sector1\n");
			lba = fs->lba_begin + fs->rootdir_first_sector + sector;
		}
        else
            return 0;
		//os_printf("in fatfs_read_sector2\n ");
    ...
    {
		//os_printf("in fatfs_read_sector3\n");
        // User target buffer passed in
        if (target)
        {
            // Calculate read address
...
int ICACHE_FLASH_ATTR fatfs_write_sector(struct fatfs *fs, uint32 cluster, uint32 sector, uint8 *target)
{
    ...
    if (fs->fat_type == FAT_TYPE_16 && cluster == 0)
    {
        ...
        {
            // Write to disk
            return media_write(lba, target, 1);
        }
        else
        {
            // Calculate write address
            ...
            // Write to disk
            return media_write(fs->currentsector.address, fs->currentsector.sector, 1);
        }
    }
    // FAT16/32 Other
    else
...
int ICACHE_FLASH_ATTR fatfs_update_timestamps(struct fat_dir_entry *directoryEntry, int create, int modify, int access)
{
 ...
 //   time(&time_now);
   ...
   // time_info = localtime(&time_now);   //bob  add time here
	//os_printf("in update time1 stamp\n");
   ...
   fat_date = fatfs_convert_to_fat_date(time_info.tm_mday, time_info.tm_mon, time_info.tm_year);
