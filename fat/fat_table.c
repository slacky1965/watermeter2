...
extern uint32 count_of_clusters;
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR int fatfs_fat_writeback(struct fatfs *fs, struct fat_buffer *pcur)
{
    if (pcur)
    {
        ...
        if (pcur->dirty)
        {
                ...
                if (!media_write(pcur->address, pcur->sector, sectors))
                    return 0;
...
//-----------------------------------------------------------------------------
static ICACHE_FLASH_ATTR struct fat_buffer *fatfs_fat_read_sector(struct fatfs *fs, uint32 sector)
{
    struct fat_buffer *last = NULL;
    struct fat_buffer *pcur = fs->fat_buffer_head;
	
	
	//os_printf("in fatfs_fat_read_sector1  secor ==%x\n ",sector);
    ...
    pcur->address = sector;
#ifdef DEBUG
	os_printf(" in fat read  fat buf head pcur address = %x\n\n", pcur->address);
#endif
	
    // Read next sector
	if (!media_read(pcur->address, pcur->sector, FAT_BUFFER_SECTORS))
    {
...
//	os_printf(" in fat read media if statement fat buf head pcur address = %x\n\n", pcur->address);
...
uint32 ICACHE_FLASH_ATTR fatfs_find_next_cluster(struct fatfs *fs, uint32 current_cluster)
{
	...
	//os_printf("in fatfs_find_next_cluster1  current cluster == %x\n\r", current_cluster);
	...
	//os_printf("in fatfs_find_next_cluster2  current cluster == %x\n\r", current_cluster);
	...
	//os_printf("in fatfs_find_next_cluster3  fat_sector_offset == %x\n\r", fat_sector_offset);
    ...
    else
    {
		...
		//os_printf("in fatfs_find_next_cluster1  next cluster  == %x\n\r", nextcluster);
        // Mask out MS 4 bits (its 28bit addressing)
        nextcluster = nextcluster & 0x0FFFFFFF;
		//os_printf("in fatfs_find_next_cluster2  next cluster  == %x\n\r", nextcluster);
        // If end of chain found
	...
	//os_printf("in fatfs_find_next_cluster3  next cluster  == %x\n\r", nextcluster);
    return (nextcluster);
}
//-----------------------------------------------------------------------------
...
void ICACHE_FLASH_ATTR fatfs_set_fs_info_next_free_cluster(struct fatfs *fs, uint32 newValue)
{
	...
	//os_printf("in fatfs_set_fs_info_next_free_cluster  newValue  ==%x\n ", newValue);
    if (fs->fat_type == FAT_TYPE_16)
        ;
    else
    {
		...
		//print_sector(fs->lba_begin + fs->fs_info_sector, *pbuf);


#ifdef DEBUG
		//print_sector(2001, pbuf);
#endif		
        // Change
		
        FAT32_SET_32BIT_WORD(pbuf,492, newValue);  //492
		
		


        fs->next_free_cluster = newValue;
		//os_printf("next free cluster == %x\n",newValue);

		//print_sector(fs->lba_begin + fs->fs_info_sector, pbuf);


#ifdef DEBUG
		os_printf("pbuf address  =%x\n\n", pbuf->address);
#endif
       media_write(pbuf->address, pbuf->sector, 1);

	  // os_printf("in fatfs_set_fs_info_next_free_cluster   pbuf->address ==%x   pbuf->sector ===%x\n ", pbuf->address, pbuf->sector);
...
int ICACHE_FLASH_ATTR fatfs_find_blank_cluster(struct fatfs *fs, uint32 start_cluster, uint32 *free_cluster)
{
	...
	if (start_cluster == 2)
	{
		...
		// go get info_sector 492  check that cluster is grater then 2 and less then  data_sectors/fs->sectors_per_cluster
		pbuf = fatfs_fat_read_sector(fs, fs->lba_begin + fs->fs_info_sector);  //read info sector

		start_cluster_temp = GET_32BIT_WORD(pbuf->ptr,492);
		
		if (start_cluster_temp > 2 && start_cluster_temp < count_of_clusters);
		{
			current_cluster = start_cluster_temp;
		}

		os_printf("start cluster ===%x\n", current_cluster);
	}

	

	os_printf("in find blank cluster start_cluster == %x\n\n\n", start_cluster);
    ...
    *free_cluster = current_cluster;
	//os_printf("in find blank cluster free_cluster == %x  found blank current_cluster =%x  \n\n\n", *free_cluster, current_cluster);
...
int ICACHE_FLASH_ATTR fatfs_fat_set_cluster(struct fatfs *fs, uint32 cluster, uint32 next_cluster)
{
    struct fat_buffer *pbuf;
    uint32 fat_sector_offset, position;
	//os_printf("in fatfs_fat_set_cluster\n");
    // Find which sector of FAT table to read
    if (fs->fat_type == FAT_TYPE_16)
...
uint32 ICACHE_FLASH_ATTR fatfs_count_free_clusters(struct fatfs *fs)
{
    ...
    for (i = 0; i < fs->fat_sectors; i++)
    {
		...
		os_printf("in fatfs_count_free_clusters\n");
        // Read FAT sector into buffer
...
void ICACHE_FLASH_ATTR fatfs_set_fs_start_sector(struct fatfs *fs, uint32 newValue)
{

	//os_printf("fatfs_set_fs_start_sector == %x\n", newValue);
	if (fs->fat_type == FAT_TYPE_16)
		;
	else
	{
		...
		//os_printf("in fatfs_set_fs_start_sector   next free cluster == %x\n", newValue);
